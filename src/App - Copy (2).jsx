    ...
    <div className="bg-gray-900 text-gray-200 min-h-screen font-sans flex flex-col">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-6 py-4 bg-gray-100/95 backdrop-blur-md border-b border-gray-300 transition-all duration-300">
        ...
        <nav className="hidden md:flex space-x-8">
          <button onClick={() => scrollToSection('home')} className="text-gray-700 hover:text-cyan-600 font-medium">Home</button>
          <button onClick={() => scrollToSection('about')} className="text-gray-700 hover:text-cyan-600 font-medium">About</button>
          <button onClick={() => scrollToSection('services')} className="text-gray-700 hover:text-cyan-600 font-medium">Services</button>
          <button onClick={() => scrollToSection('contact')} className="text-gray-700 hover:text-cyan-600 font-medium">Contact</button>
        </nav>
      ...
      <section id="home" className="relative flex flex-col items-center justify-center text-center py-32 px-6 min-h-screen bg-cover bg-center" style={{ backgroundImage: `url(${background})` }}>
        ...
        <div className="relative z-10 max-w-6xl mx-auto pt-20">
          <h1 className="text-5xl md:text-7xl font-extrabold mb-8 text-white leading-tight">
            Your Technology <span className="block bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">Evolution</span> Starts Here
          </h1>
          <p className="text-gray-300 text-xl md:text-2xl mb-10 max-w-4xl mx-auto">
      ...
      {/* ABOUT, SERVICES, CONTACT sections stay unchanged */}
